...
img_small = cv.imread('a1images/a1q5images/im03small.png')
img_large = cv.imread('a1images/a1q5images/im03.png')
...
ssd_n = np.sum(img_zoomed_nearest.astype(np.float32) - img_large_resized.astype(np.float32)) / (img_large.shape[0] * img_large.shape[1])
...
ssd_b = (np.sum(img_zoomed_bilinear.astype(np.float32) - img_large_resized.astype(np.float32))) / (img_large.shape[0] * img_large.shape[1])
